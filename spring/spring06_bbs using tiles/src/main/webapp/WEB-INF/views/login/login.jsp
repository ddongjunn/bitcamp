				...
				<form action="loginAf.do" name="frmForm" id="_frmForm" method="post">
					<table class="content_table" style="width:75%">
						...
						<tr>
							...
							<td>&nbsp;<input type="text" id="_userid" name="id" size="20px" style="border: 1px solid #dddddd" placeholder="ID">
									  <input type="checkbox" value="chk_save_id">ID저장
							</td>
						</tr>
						<tr>
							<th style="background: #eeeeee; color: #3e5fba">패스워드</th>
							<td>&nbsp;<input type="text" id="_pwd" name="pwd" size="20px" style="border: 1px solid #dddddd" placeholder="Password">									
							</td>
						</tr>
						<tr>
...
$("#chk_save_id").click(function () {
   ...
   if( $("#chk_save_id").is(":checked") ){   // 첵크가 되었을 때      
      if( $("#_userid").val().trim() == "" ){
         ...
         // id를 쿠키에 저장
         $.cookie("_userid", $("#_userid").val().trim(), { expires:7, path:'/' });
      }
   }
   else{
